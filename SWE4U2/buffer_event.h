...
public:
	...
	std::vector<std::shared_ptr<event>> process_event() override {
		if (sim_ref->b_has_capacity()) {
			...
			return std::vector<std::shared_ptr<event>>{std::make_shared<machine_b_event>(m_name, time + get_random_time(1, 5), sim_ref)};
		} else {
	...
	std::vector<std::shared_ptr<event>> process_event(product& prod) override {
		...
		if (!ret.empty()) {
			prod.time_spent_buffer = time - prod.time_spent_a;
		}
		return ret;
	}
